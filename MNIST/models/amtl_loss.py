...
class Model(nn.Module):
    ...
    def forward(self,x,y):
        ...
        pred_without_transfer = [None for _ in range(self.num_tasks)]
        pred_with_transfer = [None for _ in range(self.num_tasks)]
        loss_without_transfer = [None for _ in range(self.num_tasks)]
        ...
        for i in range(self.num_tasks):
            ...
            pred_without_transfer[i] = torch.sigmoid(self.module_dict['outlayer_without_transfer_task%d'%(i)](hidden[i]))
            loss_without_transfer[i] = F.binary_cross_entropy(pred_without_transfer[i], y[i], reduction='none')
        ...
        for i in range(self.num_tasks):
            ...
            for j in range(self.num_tasks):
                if i!=j:
                    ...
                    borrow_sigma = self.module_dict['sigma_task%d'%(j)](borrow_h)
                    ...
                    information_source = loss_without_transfer[i].mean(0,keepdims=True).detach()
                    information_target = loss_without_transfer[j].mean(0,keepdims=True).detach()
        ...
        return loss_without_transfer, loss_with_transfer, pred_without_transfer, pred_with_transfer
...
class DataParallel_Model(nn.Module):
    ...
    def forward(self,x,y):
        loss_without_transfer, loss_with_transfer, pred_without_transfer, pred_with_transfer = self.model(x,y)
        loss_without_transfer = [loss_without_transfer[i].mean() for i in range(self.num_tasks)]
        loss_with_transfer = [loss_with_transfer[i].mean() for i in range(self.num_tasks)]
        main_loss = sum(loss_without_transfer) + sum(loss_with_transfer)

        loss_dict = OrderedDict()
        loss_dict['Main Loss:'] = main_loss
        for i in range(self.num_tasks):
            loss_dict['Task %d Loss without Transfer:'%(i)] = loss_without_transfer[i]
        for i in range(self.num_tasks):
            loss_dict['Task %d Loss with Transfer:'%(i)] = loss_with_transfer[i]

        pred_dict = [OrderedDict() for _ in range(self.num_tasks)]
        for i in range(self.num_tasks):
            pred_dict[i]['without Transfer:'] = pred_without_transfer[i]
